...
export default function ProductClient({p}:{p:Product}){
  ...
  function add(){
    localStorage.setItem('tovi_cart_count',String(Number(localStorage.getItem('tovi_cart_count')||0)+1));
    setAdded(true);
  }
  ...
  return <main className="site-shell min-h-screen">
    <header className="site-header">
      ...
      <div className="header-actions"><Link href="/shop" className="bag-link">Collection</Link><Link href="/wishlist" className="icon-button"><Heart size={17}/></Link><Link href="/cart" className="icon-button"><ShoppingBag size={17}/></Link></div>
    </header>
    <section style={{display:'grid',gridTemplateColumns:'1.2fr .8fr',minHeight:'calc(100vh - 70px)'}}>
      ...
      <div style={{padding:'clamp(40px,7vw,100px)',display:'flex',flexDirection:'column',justifyContent:'center'}}>
        ...
        <p style={{color:'var(--gold)',fontSize:18}}>{p.price}</p>
        ...
        <button className="btn-gold" onClick={add} style={{border:0,cursor:'pointer'}}>{added?'Added to bag ✓':'Add to bag'} <ShoppingBag size={14}/></button>
        {added&&<Link href="/cart" className="gold-link">View shopping bag →</Link>}
      </div>
    </section>
  </main>
}
