...
const base='/tovi';
const images={
  hero:`${base}/0eddd20f8458375dadf3b78c39c675a2.jpg`,
  ...
  office:'https://images.unsplash.com/photo-1600607687920-4e2a09cf159d?auto=format&fit=crop&w=1800&q=88',
...
const products=[
  ['Luna Bouclé Sofa','KSh 89,900','Living',images.living],
  ['Atelier Dining Collection','KSh 119,000','Dining',images.dining],
  ['Noir Bedroom Suite','KSh 148,500','Bedroom',images.bedroom]
];
const inspiration=[
...
<header className="site-header">
  ...
  <div className="header-actions"><button className="icon-button" aria-label="Search"><Search size={17}/></button><Link className="icon-button" href="/wishlist" aria-label="Wishlist"><Heart size={17}/></Link><Link className="bag-link" href="/cart"><ShoppingBag size={17}/><span>Bag</span></Link><ThemeToggle/></div>
</header>
<section className="hero-premium">
  <img src={images.hero} alt="VINCE Maison luxury living room"/>
  <div className="hero-shade"/>
  <div className="hero-content">
    <div className="hero-meta"><p>Collection No. 01 / 2026</p><p>Nairobi<br/>Kenya</p></div>
    <div className="hero-bottom"><p className="eyebrow light">Furniture · Interiors · Objects</p><h1 className="display hero-title">Live<br/><i>beautifully.</i></h1><div className="hero-actions"><p>A new language of living. Sculptural furniture, tactile materials and quiet spaces, selected for homes with a point of view.</p><div className="button-row"><Link href="/shop" className="btn-gold">Explore collection <ArrowUpRight size={14}/></Link><Link href="/spaces" className="btn-ghost">Explore spaces</Link></div></div></div><div className="hero-foot"><span>Scroll to discover</span><span>01 — 04</span></div>
  </div>
</section>
...
<section className="inspiration-section"><div className="section-head"><div><p className="eyebrow">02 — The inspiration edit</p><h2 className="display">A Pinterest-worthy point of view.</h2></div><a className="muted-link" href="https://www.pinterest.com/bocadolobo/luxury-furniture/" target="_blank" rel="noreferrer">Open inspiration board ↗</a></div><div className="masonry">{inspiration.map(([title,img],i)=><Link href="/journal" className={`inspiration-card insp-${i+1}`} key={title}><img src={img} alt={title}/><div><span>{title}</span><ArrowUpRight size={14}/></div></Link>)}</div><p className="image-note">Pinterest is used here as visual direction; the photography displayed on VINCE uses the repository assets and free-to-use imagery rather than republishing random Pinterest pins.</p></section>
<section className="products-section"><div className="section-head"><div><p className="eyebrow">03 — The edit</p><h2 className="display">Pieces with presence.</h2></div><Link href="/shop" className="muted-link">Shop all →</Link></div><div className="product-showcase">{products.map(([name,price,cat,img],i)=><Link href="/shop" className={`product-piece piece-${i+1}`} key={name}><div className="product-image"><img src={img} alt={name}/><span className="piece-number">0{i+1}</span><span className="product-view">View piece <ArrowUpRight size={13}/></span></div><div className="product-info"><div><p className="product-cat">{cat}</p><h3 className="display">{name}</h3></div><p className="price">{price}</p></div></Link>)}</div></section>
