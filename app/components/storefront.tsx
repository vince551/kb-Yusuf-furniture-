...
import { ArrowUpRight, Heart, ShoppingBag, Sparkles, Star, Truck, ShieldCheck, MessageCircle, Mail, Menu, X, Home, Grid2X2, Compass, BookOpen, UserRound } from 'lucide-react';
...
export function Header(){
  ...
  return <>
    <header className="site-header">
      <Link href="/" className="brand" onClick={close}><span>KB YUSUF FURNITURE</span><small>Interior Decorations · Furniture</small></Link>
      <nav className="main-nav"><Link href="/">Home</Link><Link href="/shop">Collection</Link><Link href="/spaces">Spaces</Link><Link href="/journal">Journal</Link><Link href="/contact" className="gold">Design Services</Link></nav>
      <div className="header-actions"><Link className="icon-button" href="/wishlist" aria-label="Wishlist"><Heart size={17}/></Link><Link className="bag-link" href="/cart" aria-label={`Request bag${count?`, ${count} pieces`:''}`}><ShoppingBag size={17}/><span>Bag{count>0&&` · ${count}`}</span></Link><button className="mobile-menu-button" type="button" aria-label={menuOpen?'Close navigation':'Open navigation'} aria-expanded={menuOpen} onClick={()=>setMenuOpen(v=>!v)}>{menuOpen?<X size={20}/>:<Menu size={20}/>}</button></div>
    </header>
    {menuOpen && <div className="mobile-menu-layer" role="dialog" aria-modal="true" aria-label="Mobile navigation">
      <button className="mobile-menu-backdrop" aria-label="Close navigation" onClick={close}/>
      <aside className="mobile-menu-panel">
        <div className="mobile-menu-top"><span>KB YUSUF / MENU</span><button type="button" aria-label="Close navigation" onClick={close}><X size={18}/></button></div>
        <nav>
          <Link href="/" onClick={close}><Home size={18}/><span>Home</span><em>01</em></Link>
          <Link href="/shop" onClick={close}><Grid2X2 size={18}/><span>Collection</span><em>02</em></Link>
          <Link href="/spaces" onClick={close}><Compass size={18}/><span>Spaces</span><em>03</em></Link>
          <Link href="/journal" onClick={close}><BookOpen size={18}/><span>Journal</span><em>04</em></Link>
          <Link href="/contact" onClick={close}><Sparkles size={18}/><span>Design Services</span><em>05</em></Link>
        </nav>
        <div className="mobile-menu-links"><Link href="/wishlist" onClick={close}><Heart size={15}/> Wishlist</Link><Link href="/account" onClick={close}><UserRound size={15}/> Account</Link><Link href="/cart" onClick={close}><ShoppingBag size={15}/> Bag{count?` (${count})`:''}</Link><a href="https://wa.me/2348109730941" target="_blank" rel="noreferrer" onClick={close}><MessageCircle size={15}/> WhatsApp</a></div>
