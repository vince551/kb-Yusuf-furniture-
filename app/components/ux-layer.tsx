...
import { ArrowUp, Command, Home, Search, ShoppingBag, Sparkles, X } from 'lucide-react';
...
export default function UXLayer(){
  ...
  return <>
    ...
    {searchOpen && <div className="search-command" role="dialog" aria-modal="true" aria-label="Search VINCE Maison">
      <button className="search-backdrop" aria-label="Close search" onClick={()=>setSearchOpen(false)}/>
      <div className="search-panel">
        <div className="search-head"><div><p className="search-kicker">VINCE MAISON / DISCOVER</p><h2>Find something beautiful.</h2></div><button onClick={()=>setSearchOpen(false)} aria-label="Close"><X size={19}/></button></div>
        ...
        <div className="search-footer"><Sparkles size={15}/><span>Looking for a complete room? <Link href="/concierge" onClick={()=>setSearchOpen(false)}>Ask VINCE Concierge</Link></span></div>
      </div>
    </div>}
  </>;
}
